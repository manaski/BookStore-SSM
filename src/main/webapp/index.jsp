    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form class="form-horizontal">
		  ...
		  <div class="form-group">
		    <label class="col-sm-2 control-label">salesAmount</label>
		    <div class="col-sm-4">
			 <input type="text" name="salesAmount" class="form-control" id="salesAmount_add_input"  placeholder="0">
			      <span class="help-block"></span>
		    </div>
		   </div>
		    <div class="form-group">
		    <label class="col-sm-2 control-label">storeNumber</label>
		    <div class="col-sm-4">
			 <input type="text" name="storeNumber" class="form-control" id="storeNumber_add_input"  placeholder="0">
			      <span class="help-block"></span>
		    </div>
		   </div>
		</form>
  ...
  <div class="modal-dialog" role="document">
     ...
     <div class="modal-body">
        <form class="form-horizontal">
		  ...
		  <div class="form-group">
		    <label class="col-sm-2 control-label">publishingDate</label>
		    <div class="col-sm-4">
		     <input type="text" name="publishingDate" class="form-control" id="publishingDate_update_input"  placeholder="1991-11-01" >
			      <span class="help-block"></span>
		    </div>
		  </div>
		  <div class="form-group">
		    <label class="col-sm-2 control-label">salesAmount</label>
		    <div class="col-sm-4">
			 <input type="text" name="salesAmount" class="form-control" id="salesAmount_update_input"  placeholder="0">
			      <span class="help-block"></span>
		    </div>
		   </div>
		    <div class="form-group">
		    <label class="col-sm-2 control-label">storeNumber</label>
		    <div class="col-sm-4">
			 <input type="text" name="storeNumber" class="form-control" id="storeNumber_update_input"  placeholder="0">
			      <span class="help-block"></span>
		    </div>
		   </div>
		</form>
	...
	function validate_add_form(){
		...
		if(!regauthor.test(author)){
			//应该清空这个元素之前的样式
			show_validate_msg("#author_add_input", "error", "作者可以是1-16位中文或者1-32位英文的组合");
			return false;
		}else{
		...
		$("#bookAddModal input").each(function(){
			if($(this).val()==""){
				hasEmpty=2;
				show_validate_msg(this, "empty", "不可以为空");
				//这个返回值并不是外层函数的返回值
				return false;
			}else{
	...
	function show_validate_msg(ele,status,msg){
		...
		}else if("error" == status||"empty"==status){
		...
		$.ajax({
			...
			success:function(result){
				...
				if(result.code == 100){
					...
					//console.log(result);
					//有哪个字段的错误信息就显示哪个字段的；
					if(undefined != result.extend.errorFields.author){
						//显示作者错误信息
					...
					if(undefined != result.extend.errorFields.publishingDate){
						//显示出版日期的错误信息
						show_validate_msg("#publishingDate_add_input", "error", result.extend.errorFields.publishingDate);
					}
				}
			}
		});
	...
	function getBook(id){
		$.ajax({
			...
			success:function(result){
				...
				$("#title_update_static").text(bookData.title);
				$("#author_update_static").text(bookData.author);
				$("#price_update_input").val(bookData.price);
				$("#publishingDate_update_input").val(bookData.publishingDate);
				$("#salesAmount_update_input").val(bookData.salesAmount);
				$("#storeNumber_update_input").val(bookData.storeNumber);
			}
		});
	}
	...
	$("#book_update_btn").click(function(){
		...
		//1、检验是否为空
		var hasEmpty=1;
		$("#bookUpdateModal input").each(function(){
			if($(this).val()==""){
				hasEmpty=2;
				show_validate_msg(this, "empty", "不可以为空");
				//这个返回值并不是外层函数的返回值
		...
		$.ajax({
			...
			success:function(result){
				...
				if(result.code == 100){
					...
					if(undefined != result.extend.errorFields.publishingDate){
						//显示出版信息的错误信息
						show_validate_msg("#publishingDate_update_input", "error", result.extend.errorFields.publishingDate);
					}
					if(undefined != result.extend.errorFields.salesAmount){
						//显示出版信息的错误信息
						show_validate_msg("#salesAmount_update_input", "error", result.extend.errorFields.salesAmount);
					}
					if(undefined != result.extend.errorFields.storeNumber){
						//显示出版信息的错误信息
						show_validate_msg("#storeNumber_update_input", "error", result.extend.errorFields.storeNumber);
					}
				}
	...
	$("#book_delete_all_btn").click(function(){
		...
		//alert(titles);
		if(titles.length>0&&confirm("确认删除【"+titles+"】吗？")){
			//发送ajax请求删除
			$.ajax({
